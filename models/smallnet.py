...
class GeneratorW1(nn.Module):
    def __init__(self, args):
        ...
        self.linear2 = nn.Linear(512, 512, bias=self.bias)
        self.linear3 = nn.Linear(512, 32*5*5*self.in_channels + 32, bias=self.bias)
        self.bn1 = nn.BatchNorm1d(512)
        self.bn2 = nn.BatchNorm1d(512)
...
class GeneratorW3(nn.Module):
    def __init__(self, args):
        ...
        self.linear1 = nn.Linear(self.z, 512, bias=self.bias)
        self.linear2 = nn.Linear(512, 512, bias=self.bias)
        self.linear3 = nn.Linear(512, 32*int((self.input_size/4-3)*(self.input_size/4-3))*self.num_classes+self.num_classes, bias=self.bias)
        self.bn1 = nn.BatchNorm1d(512)
        self.bn2 = nn.BatchNorm1d(512)
...
class HyperGAN(HyperGAN_Base):
    ...
    def save_models(self, args, metrics=None):
        ...
        # path = 'saved_models/{}/hloss/{}/smallnet-{}.pt'.format(args.dataset,args.s_std, metrics)
        # path = 'saved_models/{}-diversity-loss/randn-normal-10-seed/smallnet-{}.pt'.format(args.dataset,metrics)
        # path = 'saved_models/{}-diversity-loss/new-ensemble-per-outlier-majority-vote/{}/D-3/smallnet-{}.pt'.format(args.dataset,args.s_std,metrics)
        path = 'saved_models/{}/adv-trained/random-enhanced/smallnet-{}.pt'.format(args.dataset, metrics)
